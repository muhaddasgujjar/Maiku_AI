...
**Phase 5 — COMPLETE.**
- Repo: https://github.com/muhaddasgujjar/Maiku_AI
- Auto-update (electron-updater) activates when a GitHub Release is published
- To produce an installer: `.\build.ps1` → `dist-electron\Maiku AI Setup 0.1.0.exe`

**Audio pipeline — fixes applied (2026-05-11):**
- WASAPI loopback now opens at device's NATIVE sample rate (48 kHz typical); WAV header matches → Whisper accuracy restored
- Silence detection (RMS < 150) skips Whisper call on quiet chunks → saves ~30% Groq quota
- Suggestion cadence: time-based throttle (8 s normal, 3 s on `?`-ending sentences) replaces broken % 3 count trigger
- Whisper vocabulary prompt added for tech-interview terms
- Bullet parser handles `•`, `-`, `*`, numbered formats
